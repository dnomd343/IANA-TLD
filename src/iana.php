...
class IANA { // 获取IANA顶级域名信息
    ...
    private function splitHtml($html) { // HTML页面切割
        // Get core part
    ...
    private function getHtmlTitle($str) { // 提取标题TLD字段
    ...
    private function getHtmlType($str) { // 提取TLD类型
    ...
    private function getHtmlContact($str) { // 提取联系人信息
        if ($str === '') {
            return array();
        }
        ...
        if($result['voice'] != '' && !$this->isVoice($result['voice'])) {
            die('error analyse -> contact');
        }
        if($result['fax'] != '' && !$this->isVoice($result['fax'])) {
            die('error analyse -> contact');
        }
        return $result;
    }

    private function getHtmlNS($str) { // 提取TLD名称服务器
        if ($str === '') {
            return array();
        }
    ...
    private function getHtmlInfo($str) { // 获取官网/Whois服务器信息
    ...
    private function getHtmlDate($str) { // 提取TLD注册和更新日期
    ...
    public function getTldInfo($tld, $html_content) { // 分析指定TLD数据
        $html = $this->splitHtml($html_content);
        unset($html['report']);
        if ($this->getHtmlTitle($html['title']) !== $tld) {
            die('error analyse -> title');
        }
        $info['type'] = $this->getHtmlType($html['type']);
        $info['manager'] = $this->getHtmlManager($html['manager']);
        $info['admin_contact'] = $this->getHtmlContact($html['admin']);
        $info['tech_contact'] = $this->getHtmlContact($html['tech']);
        $info['nameserver'] = $this->getHtmlNS($html['ns']);
        $web = $this->getHtmlInfo($html['info']);
        $info['website'] = $web['website'];
        $info['whois'] = $web['whois'];
        $date = $this->getHtmlDate($html['date']);
        $info['last_updated'] = $date['update'];
        $info['regist_date'] = $date['regist'];
        return $info;
    }

    public function getTlds($html_content) { // 获取IANA上所有TLD
        $html = explode('tbody>', $html_content)[1];
